    ...
    public partial class UIToggleGroup : UIToggle
    {
        ...
        public void RefreshAllToggleValues(bool animateChange = true, bool triggerValueChanged = true)
        {
            ...
            switch (mode)
            {
                ...
                case ControlMode.OneToggleOnEnforced:
                    ...
                    if (!setFirstToggleOn)
                    {
                        ...
                        if (firstToggle != null && !firstToggle.isOn)
                            firstToggle.UpdateValueFromGroup(true, false, triggerValueChanged);
                    }
                    break;
                case ControlMode.AnyToggleOnEnforced:
                    setFirstToggleOn = numberOfTogglesOn != 1;
                    if (!setFirstToggleOn)
                    {
                        foreach (UIToggle t in toggles.Where(t => t.isOn))
                            t.UpdateValueFromGroup(true, false, triggerValueChanged);
                    }
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            ...
            if (setFirstToggleOn)
            {
                ...
                GetFirstToggle()?.UpdateValueFromGroup(true, animateChange, triggerValueChanged);
            }
